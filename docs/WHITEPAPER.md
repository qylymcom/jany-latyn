...
**Cite as:** Emil Madraimov (2026). *Jany-Latyn: A Latin Orthography and Comparative Testbed for the Kyrgyz Language*, version 2026.2. https://doi.org/10.5281/zenodo.22844476
...
An alphabet is adopted by people who did not ask for it, and they meet its letters before they meet its arguments. Kazakhstan's apostrophe alphabet and then its acute one were judged in public on how they looked, and each was replaced (§8.5); Kyrgyzstan's own 2004 orthography reform was reversed when readers did not take it up [[3]](#ref3). A new letter costs little to design and a great deal to teach, and the cost falls on every reader at once.

Measured against what a Kyrgyz reader already knows, several CTA letters are new. `c` for `ж` contradicts the value `c` carries in the Latin a Kyrgyz reader meets most, the international vocabulary of *eco-*, *corp*, and *cooperation*, where it is never [dʒ] (§8.2). Dotless `ı` and dotted `İ` are letters most Kyrgyz readers have never been taught, so a correctly cased `KIRGIZ` still reads as *КИРГИЗ* (§7). `ğ` has no counterpart in any Kyrgyz writing in use since 1940. Each is familiar to a reader of Turkish, which is the familiarity the CTA was built around.

Signage and casual writing are where that is met first, by readers who have had no instruction at all, and a reader meeting an unfamiliar orthography takes in the base letters before the diacritics and reads them with the values already known. In a CTA-aligned configuration *жарыя* (announcement) is written `CARIYA`, which a reader who has not learned that `c` is [dʒ] sounds out as *кария* or *сария*; neither is the word on the sign, and nothing in the string signals the error. *Чыгуу* (exit) becomes `ÇIĞUU`, which that reader takes as `CIGUU`, the form the casual register writes outright (§10). Read with the Cyrillic values of `C` and `I`, one of the commonest words on public signs turns obscene. Canonical `ÇYGUU` can mislead by one letter, `C` for `ч`; the CTA spelling adds a second, `I` for `ы` (§7).

These costs are transitional, and end once the letters are taught; that is the answer a state-backed standard can give. The transition is long: §8.5 records Kazakhstan's completion date moving to 2031, and Uzbekistan's transition running since 1993 without finishing. An orthography is judged during that interim, and orthography development treats reader acceptance as a design criterion in its own right [[27]](#ref27).

Jany-Latyn draws instead on the two kinds of familiarity a Kyrgyz reader already has. One is the Cyrillic they read: `í` is `i` with a mark as `й` is `и` with one (§4.4), `ŋ` keeps the descending stroke of `ң` (§5.3), and length is doubled as it is in Cyrillic (§4.3). The other is the Latin they already type and see: `j` in the passport spelling *Japarov* and the place name *Jalal-Abad* (§5.1), `y` in the country's English name (§7), and an ASCII register that matches how people text now (§10). The two follow a generational line, since readers educated in Cyrillic know the first and readers who grew up on phone keyboards know the second, and the letters are chosen to be recognizable from both sides.

`ŋ` is where the two part ways. Its familiarity is of shape, to a reader of `ң`, and less of keyboards: `ŋ` is on some long-press menus (§5.3), `ñ` on nearly all of them. That is why `ñ` stays a first-class option. Everywhere else, the letter a Kyrgyz reader would guess is the letter the orthography writes.

### 12.5 The case for convergence, and the position taken

Convergence is the point of the CTA, and mutual legibility with the Turkic languages already written in Latin is a real benefit; it is one of three reasons a Latin alphabet is worth considering here at all. The other two come with any Latin orthography, whatever it aligns itself with: Latin is the script international readers already read, and the script that keyboards, encodings, and text tooling are built around (§13.4, §13.5). Only the first is bought by conformity to the CTA, which makes this a question of how much conformity rather than whether Latin.

With Kazakh the benefit is not new. The two already read each other's Cyrillic, and both sent representatives to the Baku talks that agreed the Common Turkic Alphabet, Kazakhstan with a Latin transition of its own already decreed and dated (§8.5), Kyrgyzstan without a decision to switch (§8.3, §12.1). What a transition decides is whether the link survives. If both build on a common Latin base, it is kept; if one moves and the other does not, it lasts only as long as the readers who know both scripts; if both move but diverge further than their Cyrillic did, it thins. How near the two languages drift to each other is a matter between sister languages that share their inheritance in both directions. What this section objects to is alignment with the prescribed conventions of a third language, dominant and unintelligible to both, which is what the CTA's `c` and `ı` amount to (§8.2, §12.3).

Jany-Latyn does not decline such convergence. Three of the CTA's 34 letters have no work to do in Kyrgyz (§9.5); of the 31 that remain, this orthography takes 26, `ç`, `ş`, `ö`, and `ü` among them, where agreement across the family is universal. Four are declined with an argument each: `c`, `q`, and `ğ` because each makes the orthography depend on a maintained loanword lexicon (§6, §8.2), and `ı` because the distinction it carries survives neither the casing nor the small type it has to pass through (§7). The fifth, `ñ`, is a substitution: `ŋ` continues the shape Kyrgyz wrote in 1928, and `ñ` stays a first-class option (§5.3). `Í` is the one letter Jany-Latyn has that the CTA does not, and it is there because `y` is already spent on `ы` (§4.4, §7).

How near that comes to adopting the whole standard turns on the four letters §6, §7, and §8.2 argue against. A configuration that takes all four — `c`, `q`, `ğ`, and `ı` — stands at 30 of the 31 and parts from the CTA in a single letter, and the testbed's CTA preset takes that last step to `ñ` to reach all 31 (§11.1). Declining them is what puts the canonical alphabet at 26.

Declining parts of the standard would not make Kyrgyz the exception, because no Turkic state has adopted it whole. Turkmen writes the glide `ý` for the same structural reason this orthography writes `í` (§4.4) and the velar nasal `ň` where the CTA has `ñ` [[20]](#ref20), Uzbekistan's draft keeps the digraph `ng` (§5.3), and Kazakhstan revised its own letters twice inside 2021 (§8.5). A shared Latin base has never required shared marks above it: the Latin-script languages of Europe, Germanic, Romance, Slavic, Baltic, and Uralic among them, build on the same twenty-six letters and agree on no common set of diacritics above them, and divergent spelling between neighbours is the ordinary state of affairs [[21]](#ref21). The closest precedent is at home. Kyrgyz and Kazakh Cyrillic share a base, each keeps letters of its own, and the two stay legible to each other while remaining distinct on the page, Kazakh by the letters it adds and Kyrgyz by their absence (§12.2). That is the relationship this document asks a Latin transition to keep.

The Common Turkic Alphabet could accommodate every distinction argued in this document. The price is more letters, a maintained loanword lexicon and an institution to keep it (§6, §8.2), and the teaching cost of shapes no Kyrgyz reader has met (§12.4). A state-backed standard can pay the first two. The third falls on readers no matter who pays for the rest. Jany-Latyn is built to need none of the first two, and as little of the third as it can.

A language is worth writing in the form it already uses, and a regional standard backed by a dominant partner is worth joining as far as it leaves the smaller language distinguishable and no further, since convergence pursued far enough leaves fewer distinct written languages rather than more legible ones. Those are positions, not empirical results, and neither is a defence of purity: a large share of everyday Kyrgyz vocabulary is Russian in origin, whatever follows Cyrillic will bring borrowings of its own, and none of that is at issue. What is at issue is a change that makes the language harder to find and easier to file as another, a cost paid first in machine layers and paid where nobody is watching. The other cost is paid in the open, by readers asked to learn an alphabet they did not choose (§12.4), and it applies to the letters argued here as much as to any others. Whether either cost is worth paying is for Kyrgyz speakers to decide.
...
- **Geopolitics.** Both pulls described in §12.1 and §12.2 are costs to a state-level transition, and a reason a non-state, tool-first project can do useful work that a policy campaign cannot.
- **Generational asymmetry.** The two registers of §10 are the design's answer to the generational line in §12.4: the formal register for print and the classroom, the casual register for the phone.
...
**A limit that applies to all of it.** The claim has two halves and only one can be tested now. Turkish, Azerbaijani, and Uzbek are written in Latin today, with corpora and word lists to match, so that half is measurable offline and repeatably. Kazakh is not. Its transition runs to 2031 (§8.5), so Latin Kazakh exists in decrees and specimen charts and almost nowhere else: there is no Latin Kazakh text for Latin Kyrgyz to collide with, and no body of it for an identifier to have learned from. Either could be manufactured by transliterating a Cyrillic Kazakh corpus, but that measures the transliterator that did it and not the world. The Kazakh half of §12.2 — the half §12.3 argues is the one that matters — therefore stays an argument, and the tests below say which of them reach it.

**Tests:** four, in increasing order of cost.

1. *Collision census.* Count the Kyrgyz word types whose surface form is identical to a Turkish, Azerbaijani, or Uzbek word type, under the canonical and CTA-aligned configurations. This needs word lists and no network, and it measures the mechanism directly. It does not extend to Kazakh, for the reason above.
2. *Identification against string length.* Run an off-the-shelf identifier such as fastText's `lid.176` or GlotLID over the same Kyrgyz text in both configurations at 1, 3, 5, 20, and 100 tokens, and plot accuracy against length. The prediction is that the configurations agree on paragraphs and separate as strings shorten, which is a more informative result than one misclassification rate. The confusions available to an identifier here are with the Latin Turkic languages and with Cyrillic Kazakh; the Latin Kazakh confusion §12.2 predicts is not among them and will not be until Latin Kazakh is written in quantity.
3. *Black-box observation.* Put the same Kyrgyz text in front of deployed systems in each configuration — a web search engine, an identifier, a speech or song synthesizer — and record which language each one answers in. This is the only probe that reaches the Kazakh question at all, and it is the weakest evidence in this document. These systems are opaque: an answer in Kazakh is observed, never explained, and nothing obtained this way licenses any claim about what a given system does internally or why. Each observation carries its date and whatever version the system exposes, and none of it is reproducible, since the systems change without notice. The Google search and the Suno rendering reported in §12.2 are two such observations, offered as instances and not as a result.
4. *Generative intrusion.* Prompt a model for Kyrgyz prose in each configuration and count Turkish and Kazakh lexical items per thousand words. The Kazakh count has to exclude the vocabulary the two languages inherit in common (§12.3), or it measures kinship instead of intrusion.

**What would refute it:** a negligible collision rate, or identification holding up at three tokens in both configurations. Either outcome would settle the question against §12.2, and would need to be reported if found. The Kazakh prediction has no matching refutation condition yet. That is a gap in this document and not a strength of the claim, and the transition that would close it is the same one that would make the claim matter.
