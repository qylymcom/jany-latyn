...
The second is what happens when the mark comes off. The casual register and search folding both strip diacritics, so `í` drops to `i` while `ý` would drop to `y`. Under `í`, *тыйын* and *айыл* reach ASCII as `tyiyn` and `aiyl`. Under `ý` they would arrive as `tyyyn` and `ayyl`, the collapse argument 1 introduced a separate glide letter to prevent, returning by the back door. Sorting adds a third consideration: under the ordering principle of §9.5 a marked `y` would be taught and filed at the end of the alphabet, away from the letter it alternates with.
...
Kazakhstan's 2018 alphabet is the cautionary case (§8.5), and its lesson is not that acutes fail: an acute scattered across six or seven letters reads as clutter where a single systematic use does not. `í` is one letter with one job.
...
The digraphs are not discarded; they are relocated. `janyToCyr` accepts legacy `ch`, `sh`, and `sch` on reverse conversion so that older text and casual writing convert correctly, and the testbed can render whole texts in digraph mode for comparison.
...
| :--- | :--- | :---: | :--- | :---: | :--- |
| Back | a, o, u, y | /k/ | Uvular stop [q] | **k** | *kar* [qar], *kyz* [qɯz] |
| Back | a, o, u, y | /g/ | Uvular fricative [ʁ] | **g** | *aga* [aʁa], *kyrgyz* [qɯrʁɯz] |
| Front | e, i, ö, ü | /k/ | Velar stop [k] | **k** | *kel* [kel], *küz* [kyz] |
| Front | e, i, ö, ü | /g/ | Velar stop [g] | **g** | *gül* [gyl], *egemen* [egemen] |
...
The harmony rule is also only as good as the vowels it reads. Cyrillic writes one `а` for both the back vowel and its fronted variant, so `q` mode cannot tell *äka* from *aqa*; the extended letter `ä` supplies what the rule is missing (§11.2).
...
The second is that the distinction is unreliable. `y` and `i` are unmistakable at any size, so *kyna* and *kina* never collide. Under dotless `ı` the same pair differs by a single dot, which degrades in small type, in handwriting, and on low-resolution screens. Below the formal register it disappears altogether: the casual register and search folding both reduce `ı` to `i`, which leaves *kına* and *kina* as one string in casual writing and one key in an index. With `y` the distinction survives everywhere. Once the vowel can no longer carry the difference, a consonant has to, which is why the testbed's `ı` mode switches on allophonic `q`. Adopting `ı` therefore does not cost one letter: it reopens the `q` question of §6.
...
**`í` where the CTA uses `y`.** The CTA can spend `y` on the glide because it moves `ы` to dotless `ı`; Jany-Latyn has already spent `y` on `ы` (§4.4).
...
The split also carries the lexicon dependency of §6: the CTA writes native *жазуу* as *cazuu* and the loan *журнал* as *jurnal*, and Kyrgyz Cyrillic writes one `ж` for both. With `q` and `ğ`, a CTA-aligned Kyrgyz orthography needs a loanword list at three points in its consonants, where a single `j`, `k`, and `g` need none.
...
There is also a cost that is not linguistic: spelling nearly identical to Turkish, Azerbaijani, or Uzbek removes a signal machines use to tell a smaller language apart. §12 takes that up, together with the case for convergence.
...
§9.1 and §9.2 are for anyone judging the orthography: what the converter does with loanwords, and why. §9.3 to §9.6 specify what an implementation has to reproduce, and a reader who is not writing one can read the alphabet in §9.5 and skip the rest.
...
Which losses appear in the table is partly downstream of a question §13.6 leaves open: whether loanwords are spelled from their Russian source or from Kyrgyz pronunciation. *poet → поет* and *aeroport → аеропорт* are losses under the current approximating treatment and would not be under a source-faithful one.
...
The same rule sets where the list stops. *Асхат* converts to `Ashat` and comes back as *Ашат*, and no entry can repair it: in digraph input `ashat` is also native *ашат*, which §9.1 restores first. *Исхак* was admissible because the word it displaces, *ишак*, is borrowed. The list can hold a name whose competitor is a loan and not one whose competitor is Kyrgyz, which is a firmer limit than the general caution that exception lists cost something.
...
- **Search folding.** A single `foldKey()` maps any register to one index key: `ö→o`, `ü→u`, `í→i`, `ç→c`, `ş→s`, `ŋ→n`, then casefold, which is the Unicode operation behind case-insensitive matching. For five of the six formal letters this is what standard accent-stripping already does, because `ç`, `ş`, `ö`, `ü`, and `í` split into a base letter plus a mark under Normalization Form D (NFD), the decomposed counterpart of NFC; `ä` folds to `a` the same way. `ŋ` is the exception among the formal letters and must be handled explicitly, since no amount of Unicode normalization will fold it to `n`. So are `ñ→n` and `ı→i`, for the configurations that use them.
...
- **The uvular letters fold unevenly.** `ğ` needs no rule of its own: it decomposes to `g` plus a breve under NFD and strips exactly as `ç` and `ş` do, so *bolğon* and *bolgon* land on one key. `q` is a base letter with nothing to strip, so it folds to itself, and *qyz* and *kyz* are two keys. An index holding text from both the canonical and the `q` configurations needs an explicit `q→k` rule to match them; whether that rule belongs in `foldKey` itself, at the cost of merging a distinction the configuration deliberately writes, is an open question.
- **Register equivalence holds for the strip fallback.** With the canonical ASCII mapping, the casual register *is* the folded form, so `çaí` and `cai` land on the same key. Under the transitional digraph fallback the casual form is `chai`, which folds to a different key unless the index also applies `ch→c` and `sh→s`. Any deployment that indexes digraph-style text needs that extra pair of rules; the strip fallback needs nothing.
...
This section states the alphabetical order of Jany-Latyn, the sequence a reader learns; §9.6 turns it into the sorting rules software needs.
...
What the two orders do not share is their length, and the six fewer letters are a set of decisions rather than a shortfall. Three of the CTA's letters have no work to do in Kyrgyz: `Ə` is a vowel outside the inventory, `Ū` is the Kazakh `ұ`, and `X` is redundant where `х` is written `h`. Three more are available and declined with an argument each, `C` in §8.2 and `Q` and `Ğ` in §6. Two of the remaining differences are substitutions rather than absences: `Ŋ` stands where the CTA has `Ñ`, and writing `ы` as `y` removes any need for the dotted capital `İ`. `Í` is the one letter Jany-Latyn has that the CTA does not.

`C`, `Q`, `W`, and `X` are not members. The canonical orthography never emits them: `ч` is `ç`, `ц` is `ts`, and the rest belong to other configurations or to foreign spellings.
...
Sorting is defined on the formal register; casual-register text is plain ASCII and sorts as such, so `ş` and `s` interleave there. Collation and the search folding of §9.4 pull in opposite directions, one separating what the other merges, and a system needs both.
...
| Glide | `í` (canonical); `ĭ` and `ĩ` as marked alternatives; plain `i`, the rejected barcode mapping; `y` | §4.4, §7 |
| Back unrounded vowel | `y` (canonical), `ı` | §7 |
| Uvular stop | unified `k` (canonical), allophonic `q` | §6 |
| Uvular fricative | unified `g` (canonical), allophonic `ğ` | §6 |
| Letter for `х` | `h` (canonical), `x` — every `х` written `x`, for readers who take the velar value to be the better one; not the split below | §11.2 |
...
Three couplings in that table belong to the testbed rather than to the mapping. Selecting `ı` switches the uvular options to `q` and `ğ`, and switching back to `y` restores `k` and `g`, returning the glide to `í` if it was `y` and leaving it alone otherwise. Selecting `q` on its own carries `ğ` with it, and reverting to `k` takes it back off. Each coupling only seeds the option, so `ı` with `k`, and `q` without `ğ`, both remain reachable by setting the option afterwards. The `y` glide is offered only while `ы` is written `ı`, since otherwise the two would collide. A fourth pairing is warned about rather than prevented: selecting the `ĩ` glide while the velar nasal is `ñ` puts the same tilde on two letters for two unrelated jobs, so the testbed says so and lets the reader judge the result. The engine itself imposes none of these rules: every combination of options is a valid call.
...
`w` fails the same way, because intervocalic [w] is largely the predictable realization of /b/ rather than a contrast, and Principle 3 says predictable allophones do not get letters, the same reasoning that unified `k` in §6. If `w` earns a place, loanwords will be what earns it.
...
### 12.1 The politics of leaving Cyrillic
...
### 12.2 What a shared Turkic standard costs a smaller language
...
### 12.3 Related languages, shared letters
...
Signage and casual writing are where that is met first, by readers who have had no instruction at all, and a reader meeting an unfamiliar orthography takes in the base letters before the diacritics and reads them with the values already known. In a CTA-aligned configuration *жарыя* (announcement) is written `CARIYA`, which a reader who has not learned that `c` is [dʒ] sounds out as *кария* or *сария*; neither is the word on the sign, and nothing in the string signals the error. *Чыгуу* (exit) becomes `ÇIĞUU`, which that reader takes as `CIGUU`, the form the casual register writes outright. Read with the Cyrillic values of `C` and `I`, one of the commonest words on public signs turns obscene. Canonical `ÇYGUU` can mislead by one letter, `C` for `ч`; the CTA spelling adds a second, `I` for `ы`.
...
Jany-Latyn draws instead on the two kinds of familiarity a Kyrgyz reader already has. One is the Cyrillic they read: `í` is `i` with a mark as `й` is `и` with one, `ŋ` keeps the descending stroke of `ң`, and length is doubled as it is in Cyrillic. The other is the Latin they already type and see: `j` in the passport spelling *Japarov* and the place name *Jalal-Abad*, `y` in the country's English name, and an ASCII register that matches how people text now. The two follow a generational line, since readers educated in Cyrillic know the first and readers who grew up on phone keyboards know the second, and the letters are chosen to be recognizable from both sides.

`ŋ` is where the two part ways. Its familiarity is of shape, to a reader of `ң`, and less of keyboards: `ŋ` is on some long-press menus, `ñ` on nearly all of them. That is why `ñ` stays a first-class option. Everywhere else, the letter a Kyrgyz reader would guess is the letter the orthography writes.

### 12.5 The case for convergence, and the position taken
...
- **Generational asymmetry.** Readers educated in Cyrillic and readers raised on phone keyboards start from different letters, which is what the two registers are for: the formal one for print and the classroom, the casual one for the phone.
...
**Claim:** orthographic divergence from the Latin-script Turkic standards keeps Kyrgyz identifiable to machines, and the effect grows as the text gets shorter (§12.2).

**Scope:** of the two comparisons in §12.2, only the one with Turkish, Azerbaijani, and Uzbek can be tested now, because those languages are written in Latin today. Latin Kazakh, the nearer case (§12.3), exists in decrees and specimen charts and will not be written in quantity before its transition ends in 2031 (§8.5). Transliterating a Cyrillic Kazakh corpus would test the transliterator rather than the language, so the Kazakh comparison stays an argument, and only the third test below reaches it.

**Tests:** four, in increasing order of cost.

1. *Collision census.* Count the Kyrgyz word types identical to a Turkish, Azerbaijani, or Uzbek word type under the canonical and CTA-aligned configurations. It needs only word lists and measures the mechanism directly.
...
- Whether `foldKey` should fold `q` to `k` (§9.4).
